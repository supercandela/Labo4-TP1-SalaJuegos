...
export class MayorOMenorComponent implements OnInit, OnDestroy {
  ...
  ngOnInit () {
    this.subMazo = this.cartasService.getMazoId()
    .subscribe(mazo => {
  ...
  pedirCartas () {
    this.subCartas = this.cartasService.getCartas(this.idMazo).subscribe(cartas => {
      this.cartas = cartas;
  ...
  reiniciarPartida () {
    ...
    this.cartasService.mezclarCartas(this.idMazo).subscribe( () => this.pedirCartas() );
  }
